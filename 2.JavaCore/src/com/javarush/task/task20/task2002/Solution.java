...
public class Solution {
    public static void main(String[] args) throws Exception {
        ...
        try {
            File yourFile = new File("/home/phbzr/Рабочий стол/JAVA/loler2.txt");
            OutputStream outputStream = new FileOutputStream(yourFile);
            InputStream inputStream = new FileInputStream(yourFile);

            JavaRush javaRush = new JavaRush();
            User user1 = new User();
            user1.setFirstName("TOLIK");
            user1.setLastName("Ebolik");
            user1.setMale(true);
            Date date1 =new Date();
            date1.setYear(1995);
            date1.setMonth(12);
    ...
    public static class JavaRush {
        ...
        public void save(OutputStream outputStream) throws Exception {
            ...
            if (!users.isEmpty()){
                for (User user : users)
                {
                    outputStreamWriter.write(user.getFirstName() + " " + user.getLastName() + " " +user.isMale()
                             + " " + Long.toString(user.getBirthDate().getTime()) + " " + user.getCountry() + "\n");
                }
